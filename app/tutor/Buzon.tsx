...
export default function Buzon() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        ...
        const q = query(collection(db, 'citas'), where('tutorId', '==', currentUser.uid));
  ...
  return (
    <>
      ...
      <View className='h-screen bg-grey/20 p-10'>
        {loading ? (
          ...
          messages.map((message) => (
            <View key={message.id} className='mb-5 p-4 bg-white rounded-lg shadow-lg'>
              ...
              <Text className='text-md text-gray/20'>Profesor: {message.profesorId}</Text>
